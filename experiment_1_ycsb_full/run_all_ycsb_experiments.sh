...
#   6. tiered_indexing_zxj — 3T Two-Tree baseline [Zhou+Hao+Yu+Stonebraker VLDB-J'25]
#
# YCSB workloads (6):
...
#
# Total: 6 × 6 × 5 = 180 experiment groups
#
#
# Usage:
...
VARIANTS=(two_level page_only lru dram_ssd dram_ssd_unconstrained tiered_indexing_zxj)
...
run_one() {
    ...
    case "$variant" in
        ...
        dram_ssd_unconstrained)
            ...
            ;;
        tiered_indexing_zxj)
            admission_mode="tiered-indexing-zxj"
            extra_flags=(
                "${CXL_FLAGS[@]}"
                --dram_buffer_pool_gib="$DRAM_BP_PAGE_ONLY"
                --dram_recordcache_gib=0
                --ti_lazy_migration_pct=100
            )
            ;;
        *)
