...
int main(int argc, char** argv)
{
   ...
   // use "lru" so the old policy-level simulation is not triggered on B+Tree accesses.
   ...
   if (is_3t_mode) {
      ...
      crm.scheduleJobSync(0, [&]() {
         table_3t = TwoTreeLeanStoreAdapter<YCSBTable>(db, "YCSB_A_TEST", hot_partition_gib);
      });
      print_pass("LeanStore initialized (3T Two-Tree, hot_gib=" +
                 std::to_string(hot_partition_gib) + ")");
   } else if (is_bf_tree_mode) {
      crm.scheduleJobSync(0, [&]() {
         table_bf = BfTreeLeanStoreAdapter<YCSBTable>(db, "YCSB_A_TEST", bf_tree_mini_page_gib, static_cast<u16>(FLAGS_test_payload_size_bytes));
      });
   ...
   if (FLAGS_test_warmup_lookups > 0) {
      ...
      if (is_3t_mode) {
         warmup_stats = run_lookup_phase(crm, table_3t, cfg, generator,
                                         FLAGS_test_warmup_lookups, /*measured=*/false);
      } else if (is_bf_tree_mode) {
         warmup_stats = run_lookup_phase(crm, table_bf, cfg, generator,
                                         FLAGS_test_warmup_lookups, /*measured=*/false);
      } else {
         warmup_stats = run_lookup_phase(crm, table, cfg, generator,
                                         FLAGS_test_warmup_lookups, /*measured=*/false);
      }
   ...
   // Phase 3: measured YCSB-A run
   print_phase("Phase 3: Measured Lookup (YCSB-A, 50% Read, 50% Update)");
   LookupRunStats stats;
   if (is_3t_mode) {
      stats = run_lookup_phase(crm, table_3t, cfg, generator,
                               FLAGS_test_measure_lookups, /*measured=*/true);
   } else if (is_bf_tree_mode) {
      stats = run_lookup_phase(crm, table_bf, cfg, generator,
                               FLAGS_test_measure_lookups, /*measured=*/true);
   } else {
      stats = run_lookup_phase(crm, table, cfg, generator,
                               FLAGS_test_measure_lookups, /*measured=*/true);
   }
