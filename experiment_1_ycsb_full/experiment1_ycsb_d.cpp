         ...
         while (local_done < quota) {
            ...
            jumpmuTry() {
               ...
               if (op < 0.95) {
                  ...
                  // Use runtime_insert (not insert_var): under 3T/bf-tree the
                  // latter is a bulk-load shortcut that routes around the
                  // hot/cold tiering logic; using it for warmup/measure inserts
                  // shoves new keys into cold, then every read-latest pays for
                  // a cold->hot migration. runtime_insert dispatches correctly:
                  // baseline LeanStore aliases it to insert_var (no change).
...
int main(int argc, char** argv)
{
   ...
   if (is_3t_mode) {
      ...
      crm.scheduleJobSync(0, [&]() {
         table_3t = TwoTreeLeanStoreAdapter<YCSBTable>(db, "YCSB_D_TEST", hot_partition_gib);
      });
      print_pass("LeanStore initialized (3T Two-Tree, hot_gib=" +
                 std::to_string(hot_partition_gib) + ")");
   } else if (is_bf_tree_mode) {
      crm.scheduleJobSync(0, [&]() {
         table_bf = BfTreeLeanStoreAdapter<YCSBTable>(db, "YCSB_D_TEST", bf_tree_mini_page_gib, static_cast<u16>(FLAGS_test_payload_size_bytes));
      });
