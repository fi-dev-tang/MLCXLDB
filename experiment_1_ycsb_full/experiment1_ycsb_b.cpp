...
int main(int argc, char** argv)
{
   ...
   if (is_3t_mode) {
      ...
      crm.scheduleJobSync(0, [&]() {
         table_3t = TwoTreeLeanStoreAdapter<YCSBTable>(db, "YCSB_B_TEST", hot_partition_gib);
      });
      print_pass("LeanStore initialized (3T Two-Tree, hot_gib=" +
                 std::to_string(hot_partition_gib) + ")");
   } else if (is_bf_tree_mode) {
      crm.scheduleJobSync(0, [&]() {
         table_bf = BfTreeLeanStoreAdapter<YCSBTable>(db, "YCSB_B_TEST", bf_tree_mini_page_gib, static_cast<u16>(FLAGS_test_payload_size_bytes));
      });
