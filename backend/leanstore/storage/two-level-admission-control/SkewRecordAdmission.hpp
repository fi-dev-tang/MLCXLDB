...
//
// Future Optimization:
// Can use std::atomic<uint64_t>[NUM_BITMAP_WORDS] instead of heavy-mutex on std::bitset.
...
//
// HISTORICAL NOTE on the earlier "192" value:
//   The original constant assumed 4 KiB pages. cxl-WT-comparison actually uses
//   16 KiB pages (BufferFrame.hpp: PAGE_SIZE = 16 KiB), and index-only TPC-C
//   tables (neworder_t, order_wdc_t, customerwdl_t) have payload=0, so a single
//   leaf can pack ~380 slots right before splitting. Writing
//   already_promoted_slot_set[slot_id / 64] with slot_id >= 192 overflowed the
//   3-word bitmap and corrupted the next std::unordered_map hash-node, which
//   produced sporadic SIGSEGVs inside ClearPromotedSlot / find() during D1.
// FIX:
//   MAX_SLOTS_PER_PAGE bumped to 512 — strictly above worst-case real leaf
//   capacity for any TPC-C table on 16 KiB pages with ~30% headroom.
//   NUM_BITMAP_WORDS is derived so the bitmap can never be undersized if
//   MAX_SLOTS_PER_PAGE is later bumped again.
...
struct HotPageCandidate{
    ...
    std::atomic<u64> distinct_accessed_slot_set[NUM_BITMAP_WORDS];   // Tracks which slots have been accessed
    ...
    u64 GetDistinctAccessedSlotCount() const {
        ...
        for (int i = 0; i < NUM_BITMAP_WORDS; i++) {
            total += __builtin_popcountll(
                distinct_accessed_slot_set[i].load(std::memory_order_relaxed));
        }
        return total;
    }
    ...
    u64 GetAlreadyPromotedSlotCount() const{
        ...
        for (int i = 0; i < NUM_BITMAP_WORDS; i++) {
            total += __builtin_popcountll(
                already_promoted_slot_set[i].load(std::memory_order_relaxed));
        }
        return total;
    }

    // All five per-slot accessors below guard slot_id against MAX_SLOTS_PER_PAGE
    // at runtime. The constructor's `page_slot_num <= MAX_SLOTS_PER_PAGE`
    // assertion is compiled out under RelWithDebInfo / Release, so this runtime
    // guard is the only thing that prevents the OOB write that previously
    // corrupted adjacent _Hash_node memory and produced SIGSEGVs inside
    // unordered_map traversal. Branches are predicted not-taken in the steady
    // state (AddCandidate already rejects out-of-range pages).
...
public:
    ...
    bool AddCandidate(u64 page_id,
                      ...
                      leanstore::storage::BufferFrame* cxl_bf,
                      bool is_in_dram){
        // Hard upper-bound guard: HotPageCandidate's slot bitmaps cover
        // [0, MAX_SLOTS_PER_PAGE). If the caller's page somehow has more
        // slots than that (e.g. future schema or page-size changes), refuse
        // to admit so the per-slot bitmap accessors can never go OOB.
        // With MAX_SLOTS_PER_PAGE = 512 this branch is dead on current TPC-C
        // workloads (worst-case neworder leaf ≈ 380 slots), so admission
        // throughput and hit rate are unchanged in practice.
        if (page_slot_num == 0 || page_slot_num > MAX_SLOTS_PER_PAGE) {
            return false;
        }
        const u16 shard_idx = getShardIndex(page_id);
    ...
    // Returns the vector of decisions <page_id, action, hot_slot_ids>
    //------------------------------------------------------------------------------------------------
    struct PromotionDecision{
        enum class Action : u8 { PROMOTE_FULL_PAGE = 0, PROMOTE_RECORDS = 1, DEMOTE_FULL_PAGE = 2 };
        u64 page_id;
        leanstore::storage::BufferFrame* cxl_bf;
        Action action;
        std::vector<u16> hot_slot_ids;      // Useful when page is skew, action == PROMOTE_RECORDS
    };
    ...
    struct CandidateSnapshot {
        ...
        u64 GetDistinctAccessedSlotCount() const {
            u64 total = 0;
            for (int i = 0; i < NUM_BITMAP_WORDS; i++) total += __builtin_popcountll(accessed_bits[i]);
            return total;
        }
        u64 GetAlreadyPromotedSlotCount() const {
            u64 total = 0;
            for (int i = 0; i < NUM_BITMAP_WORDS; i++) total += __builtin_popcountll(promoted_bits[i]);
            return total;
        }
    };
    ...
    std::vector<PromotionDecision> CheckAndPromote(u64 l1_fine_threshold,
        ...
        for (const auto& snap : snapshots) {
            ...
            if (l1_rule_active && !is_scan_workload) {
                ...
                if (!strong_hot_slots.empty()) {
                    decisions.push_back({page_id, snap.cxl_bf, PromotionDecision::Action::PROMOTE_RECORDS, strong_hot_slots});
            ...
            if (per_page_visits >= effective_max_per_page_visits && (!has_ratio_skew_slot || is_scan_workload)) {
                decisions.push_back({page_id, snap.cxl_bf, PromotionDecision::Action::PROMOTE_FULL_PAGE, {}});
            ...
            if (has_ratio_skew_slot) {
                if (!ratio_new_hot_slots.empty()) {
                    decisions.push_back({page_id, snap.cxl_bf, PromotionDecision::Action::PROMOTE_RECORDS, ratio_new_hot_slots});
        ...
        for (const auto& d : decisions) {
            if (d.action == PromotionDecision::Action::PROMOTE_RECORDS)
    ...
    std::vector<PromotionDecision> CollectAllAsFullPagePromotions() {
        ...
        for (u16 shard_idx = 0; shard_idx < NUM_CANDIDATE_SHARDS; shard_idx++) {
            std::shared_lock<std::shared_mutex> lock(shards[shard_idx].mutex);
            for (const auto& [pid, cand] : shards[shard_idx].map) {
                decisions.push_back({pid, cand.cxl_bf, PromotionDecision::Action::PROMOTE_FULL_PAGE, {}});
            }
        }
        return decisions;
    }
