...
public:
    ...
    // WorkerThread calling:(Multi-Producer)
    // When Worker thread has logically deleted a RecordCacheEntry
    // RecordCacheType::LogicallyDeletedButStillInHashTable = 0b011,    011: Logical deleted, but still in hash table(wait for Forward_epoch thread)
