...
//====================================================================================================
    ...
    RecordCacheEntry* evictOneFromSieveFIFO(
        ...
        while(scanned_item_num < max_scan_num && !sieve_fifo_queue.empty()){
            ...
            if(current_entry -> visited.exchange(false, std::memory_order_acq_rel)){
    ...
    // [FIX-C v2] drainStateRemovedOnly:
    ...
    // Entries in any other state (including ReadOnly 0b000) are KEPT in
    // their original FIFO order, with their visited bits LEFT UNTOUCHED.
    //
    // This is the proactive-cleanup path used when SIEVE is woken purely
    // because the pending-dead-entry counter exceeded the threshold (i.e.
    // slab is NOT yet at the watermark). In that situation we must not
    // consume the second-chance signal of real hot ReadOnly entries, nor
    // disturb their FIFO position — only release entries proven dead.
    //
