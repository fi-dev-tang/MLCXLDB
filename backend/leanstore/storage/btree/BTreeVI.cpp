...
OP_RESULT BTreeVI::lookup(u8* o_key, u16 o_key_length, function<void(const u8*, u16)> payload_callback)
{
   ...
   if(FLAGS_cxl_tiering_enabled && BMC::global_bf -> global_record_cache != nullptr){
      ...
      if(BMC::global_bf -> global_record_cache -> tryLookupInRecordCache(static_cast<u16>(this->dt_id), key_span, payload_callback, worker_id)){
...
OP_RESULT BTreeVI::lookupPessimistic(u8* key_buffer, const u16 key_length, function<void(const u8*, u16)> payload_callback)
{
   ...
   jumpmuTry()
   {
      ...
      // older snapshot TXs. The trap was a development-time assertion, not load-bearing logic.
      // TwoTreeAdapter's update1/lookup1 handle NOT_FOUND on the cold path via `found_cold`.
      // -------------------------------------------------------------------------------------
                                         ...
                                         UpdateSameSizeInPlaceDescriptor& update_descriptor)
{
   ...
   jumpmuTry()
   {
      ...
      if (ret != OP_RESULT::OK) {
         ...
         // probes hot, then calls updateSameSizeInPlace, but another worker's
         // evict_a_bunch may have physically removed K from hot in between. The caller
         // is prepared to fall through to the cold-tree path on NOT_FOUND.
         jumpmu_return ret;
      }
      // -------------------------------------------------------------------------------------
      // Record is found
   restart : {
      ...
      if (FLAGS_cxl_tiering_enabled) {
         ...
         if (is_in_dram) {
            BMC::global_bf->diag.dram_buffer_pool_hit.fetch_add(1, std::memory_order_relaxed);
         } else if (BMC::global_bf->isInCXL(iterator.leaf.bf)) {
            BMC::global_bf->diag.cxl_buffer_pool_hit.fetch_add(1, std::memory_order_relaxed);
         }
      }
      COUNTERS_BLOCK()
      ...
      if (tuple.tuple_format == TupleFormat::FAT_TUPLE_DIFFERENT_ATTRIBUTES) {
         ...
         if (FLAGS_cxl_tiering_enabled && BMC::global_bf->global_record_cache != nullptr) {
            ...
            BMC::global_bf->global_record_cache->tryInvalidateForRemove(
                static_cast<u16>(this->dt_id), key_span, cr::Worker::my().worker_id);
         }
...
{
   const bool instant_visible = (cr::activeTX().current_tx_mode == TX_MODE::INSTANTLY_VISIBLE_BULK_INSERT);
   // [3T-fairness] In INSTANTLY_VISIBLE_BULK_INSERT mode (used by TwoTreeAdapter
   // for hot/cold migration), we deliberately bypass user-tx WAL: the inserted
   // tuple is stamped tx_ts=MSB|0 so all readers see it, and there's no undo
   // semantics — if the user tx aborts later, the migrated tuple must survive
   // (it represents committed data that was just relocated). Logging it under
   // the user tx would cause undo to try removeCurrent on a key that may have
   // already been re-migrated by another worker.
   if (!instant_visible) {
      cr::activeTX().markAsWrite();
   }
   cr::Worker::my().logging.walEnsureEnoughSpace(PAGE_SIZE * 1);
   ...
   while (true) {
      jumpmuTry()
      {
         ...
         if (ret == OP_RESULT::DUPLICATE) {
            ...
            // [3T-fairness] Original ensure(false) removed: 3T's eviction may try to
            // re-insert a key that survived a prior up-migration (the cold remove was
            // racy or the bulk-loaded sentinel is still in the leaf). Surface as
            // DUPLICATE so the caller (evict_a_bunch) can skip this victim and move on.
         ...
         // -------------------------------------------------------------------------------------
         // WAL (skipped for migration inserts — see comment at top of function)
         if (!instant_visible) {
...
OP_RESULT BTreeVI::remove(u8* o_key, u16 o_key_length)
{
   ...
   // [3T-fairness] Migration removes (paired with INSTANTLY_VISIBLE inserts in
   // TwoTreeAdapter) must not mark the user tx as a write tx — the user tx may
   // be read-only, and adding spurious writes pulls it into the commit/abort
   // path. The vi_fremove fast-path below already skips WAL, so no undo entry
   // is created either.
   const bool instant_visible = (cr::activeTX().current_tx_mode == TX_MODE::INSTANTLY_VISIBLE_BULK_INSERT);
   if (!instant_visible) {
      cr::activeTX().markAsWrite();
   ...
   jumpmuTry()
   {
      ...
      if (ret != OP_RESULT::OK) {
         ...
         // -------------------------------------------------------------------------------------
         // [3T-fairness] Soft NOT_FOUND under SI: 3T's TwoTreeAdapter::erase probes
         // hot/cold then calls remove, but evict_a_bunch may physically remove K in
         // between (vi_fremove fast path). The probe-then-act TOCTOU is legitimate
         // under 3T's migration model; the caller treats NOT_FOUND as "row already
         // gone — erase is a no-op". Original explainWhen(atLeastSI) over-asserted.
         jumpmu_return OP_RESULT::NOT_FOUND;
      }
      // -------------------------------------------------------------------------------------
      // [3T-fairness] vi_fremove (physical remove, no tombstone) is required by
      // 3T's migration paths so that the paired reverse-direction re-insert
      // doesn't collide with a stale tombstone. But applying it to USER erases
      // breaks SI: a concurrent reader's scan would see the row in its snapshot
      // yet the row would be physically gone, so its follow-up erase/lookup
      // returns NOT_FOUND instead of ABORT_TX. Gate the fast path on
      // INSTANTLY_VISIBLE so only migration removes are physical; user erases
      // fall through to the SI-correct tombstone path below.
      if (FLAGS_vi_fremove && instant_visible) {
      ...
      // [3T-fairness] Two workers racing on the same key (e.g. tpcc delivery on
      // neworder) can both probe the row, then the first commits a tombstone,
      // then the second's remove finds is_removed=true. Under SI this is a real
      // write-write conflict — return ABORT_TX so the user tx retries. Original
      // ensure(!is_removed) crashed instead, which was too strict for 3T's
      // probe-then-act erase pattern.
      if (chain_head.is_removed) {
         jumpmu_return cr::activeTX().atLeastSI() ? OP_RESULT::ABORT_TX : OP_RESULT::NOT_FOUND;
      }
      // -------------------------------------------------------------------------------------
      ...
      if (FLAGS_cxl_tiering_enabled && BMC::global_bf->global_record_cache != nullptr) {
         u64 worker_id = cr::Worker::my().worker_id;
         std::span<const u8> key_span(o_key, o_key_length);
         BMC::global_bf->global_record_cache->tryInvalidateForRemove(static_cast<u16>(this->dt_id), key_span, worker_id);
         if (BMC::global_bf->global_admission_control != nullptr) {
            u64 page_id = iterator.leaf.bf->header.pid;
            BMC::global_bf->getGlobalAdmissionControl().OnRecordInvalidated(page_id, static_cast<u16>(iterator.cur));
         }
   ...
   switch (entry.type) {
      case WAL_LOG_TYPE::WALInsert: {  // Assuming no insert after remove
         ...
         jumpmuTry()
         {
            ...
            OP_RESULT ret = iterator.seekExact(key);
            if (ret != OP_RESULT::OK) {
               // [3T-fairness] 3T's evict_a_bunch may physically remove K from
               // this tree (vi_fremove fast path) before the user tx aborts.
               // MVCC guarantee: migration copied the SI-visible (pre-insert)
               // state to the sibling tree, so skipping this undo is correct —
               // the rolled-back state already exists in the other tree. The
               // original ensure() crashed because it didn't anticipate 3T.
               jumpmu_return;
            }
            ret = iterator.removeCurrent();
            ensure(ret == OP_RESULT::OK);
      ...
      case WAL_LOG_TYPE::WALUpdate: {
         ...
         jumpmuTry()
         {
            ...
            OP_RESULT ret = iterator.seekExact(key);
            if (ret != OP_RESULT::OK) {
               // [3T-fairness] evict_a_bunch may have physically removed K
               // (vi_fremove). MVCC: the SI-visible pre-update value was
               // copied to the sibling tree before removal, so skipping this
               // undo is safe — the rolled-back state lives there.
               jumpmu_return;
            }
      ...
      case WAL_LOG_TYPE::WALRemove: {
         ...
         jumpmuTry()
         {
            ...
            if (ret != OP_RESULT::OK) {
               // [3T-fairness] evict_a_bunch may have physically removed K
               // before this WALRemove undo runs. MVCC: the SI-visible
               // pre-remove value was copied to the sibling tree, so the
               // rolled-back state already exists there — safe to skip.
               jumpmu_return;
            }
            // Resize
