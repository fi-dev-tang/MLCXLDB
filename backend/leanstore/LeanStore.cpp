...
{
   std::fprintf(stdout, "[registerBTreeLL] enter name=%s\n", name.c_str()); std::fflush(stdout);
   assert(btrees_ll.find(name) == btrees_ll.end());
   auto& btree = btrees_ll[name];
   std::fprintf(stdout, "[registerBTreeLL] %s: map_insert done\n", name.c_str()); std::fflush(stdout);
   DTID dtid = DTRegistry::global_dt_registry.registerDatastructureInstance(0, reinterpret_cast<void*>(&btree), name);
   std::fprintf(stdout, "[registerBTreeLL] %s: dtid=%lu, before btree.create\n", name.c_str(), (unsigned long)dtid); std::fflush(stdout);
   btree.create(dtid, config);
   std::fprintf(stdout, "[registerBTreeLL] %s: btree.create done, exit\n", name.c_str()); std::fflush(stdout);
   return btree;
}
...
{
   std::fprintf(stdout, "[registerBTreeVI] enter name=%s\n", name.c_str()); std::fflush(stdout);
   assert(btrees_vi.find(name) == btrees_vi.end());
   auto& btree = btrees_vi[name];
   std::fprintf(stdout, "[registerBTreeVI] %s: map_insert done\n", name.c_str()); std::fflush(stdout);
   DTID dtid = DTRegistry::global_dt_registry.registerDatastructureInstance(2, reinterpret_cast<void*>(&btree), name);
   std::fprintf(stdout, "[registerBTreeVI] %s: dtid=%lu, before graveyard registerBTreeLL\n", name.c_str(), (unsigned long)dtid); std::fflush(stdout);
   auto& graveyard_btree = registerBTreeLL("_" + name + "_graveyard", {.enable_wal = false, .use_bulk_insert = false});
   std::fprintf(stdout, "[registerBTreeVI] %s: graveyard done, before btree.create\n", name.c_str()); std::fflush(stdout);
   btree.create(dtid, config, &graveyard_btree);
   std::fprintf(stdout, "[registerBTreeVI] %s: btree.create done, exit\n", name.c_str()); std::fflush(stdout);
   return btree;
}
// -------------------------------------------------------------------------------------
