...
// [v4-a] Skip 2B dt_id prefix on RC keys (single-table workloads only).
DECLARE_bool(rc_skip_dt_id_prefix);
// [v4-a] Cap on optimistic SeqLock reread loop in tryLookupInRecordCache.
DECLARE_uint32(rc_seqlock_retry_max);
...
DECLARE_uint64(ti_lazy_migration_pct);
...
//     col_num is auto-derived in the Wrapper ctor from
//     (cxl_gib + dram_bp + dram_rc) -- the page-ID range that actually
//     enters admission. page_cms_col_num is the floor/fallback only.
