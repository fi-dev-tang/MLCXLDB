  ...
  private:
   ...
   void delivery(Integer w_id, Integer carrier_id, Timestamp datetime)
   {
      for (Integer d_id = 1; d_id <= 10; d_id++) {
         ...
         // -------------------------------------------------------------------------------------
         // c_id must be init: lookup1 was softened to silently return on
         // NOT_FOUND (for YCSB-D's racy read-latest). If the order is
         // missing here, the callback never fires and an uninitialized c_id
         // would propagate into customer.update1 → EnsureFailed (N4).
         Integer ol_cnt = minInteger, c_id = minInteger;
         if (manually_handle_isolation_anomalies) {
            ...
            if (ol_cnt == minInteger || c_id == minInteger) {
               fprintf(stderr, "[N4-DIAG] delivery skipped, order missing for w=%d d=%d o=%lld\n",
                       w_id, d_id, (long long)o_id);
               continue;
            }
            // [N4-DIAG] Catch storage-returned garbage c_id before customer.update1 crashes.
            // TPC-C invariant: c_id ∈ [1, 3000] per (w,d). If we see anything else,
            // order.lookup1 returned a corrupt order_t (cache/storage corruption).
            if (c_id < 1 || c_id > 3000) {
               fprintf(stderr, "[N4-DIAG] BAD c_id from order.lookup1: w=%d d=%d o=%lld -> c_id=%d (ol_cnt=%d) — skipping\n",
                       w_id, d_id, (long long)o_id, c_id, ol_cnt);
               continue;
            }
         }
         // -------------------------------------------------------------------------------------
